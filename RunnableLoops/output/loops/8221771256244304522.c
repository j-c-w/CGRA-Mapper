...
int
fn (const uint8_t * cm, int dstStride, int srcStride, int i, uint8_t * dst,
    const uint8_t * src)
{
  const int w = 16;
  for (i = 0; i < w; i++)
	...
	(((dst[9 * dstStride]) +
      ...
      dst[10 * dstStride] =
	(((dst[10 * dstStride]) +
	  cm[(((src10 + src11) * 20 - (src9 + src12) * 6 +
	       (src8 + src13) * 3 - (src7 + src14)) + 16) >> 5] + 1) >> 1);
      dst[11 * dstStride] =
	(((dst[11 * dstStride]) +
	  cm[(((src11 + src12) * 20 - (src10 + src13) * 6 +
	       (src9 + src14) * 3 - (src8 + src15)) + 16) >> 5] + 1) >> 1);
      dst[12 * dstStride] =
	(((dst[12 * dstStride]) +
	  cm[(((src12 + src13) * 20 - (src11 + src14) * 6 +
	       (src10 + src15) * 3 - (src9 + src16)) + 16) >> 5] + 1) >> 1);
      dst[13 * dstStride] =
	(((dst[13 * dstStride]) +
	  cm[(((src13 + src14) * 20 - (src12 + src15) * 6 +
	       (src11 + src16) * 3 - (src10 + src16)) + 16) >> 5] + 1) >> 1);
      dst[14 * dstStride] =
	(((dst[14 * dstStride]) +
	  cm[(((src14 + src15) * 20 - (src13 + src16) * 6 +
	       (src12 + src16) * 3 - (src11 + src15)) + 16) >> 5] + 1) >> 1);
      dst[15 * dstStride] =
	(((dst[15 * dstStride]) +
	  cm[(((src15 + src16) * 20 - (src14 + src16) * 6 +
	       (src13 + src15) * 3 - (src12 + src14)) + 16) >> 5] + 1) >> 1);
      dst++;
      src++;
}}
