...
int
fn (uint64_t * sat, const int src_linesize, uint64_t square_sum,
    const int radius, uint8_t * dst, const uint8_t * src,
    const int sat_linesize, int lower_y, uint64_t var, int lower_x,
    uint64_t sum, uint64_t * square_sat, const int sigma, int y, int x,
    uint64_t mean, int dist_y, int count, const int width,
    const int dst_linesize, int higher_x, int higher_y)
{
  for (x = 0; x < width; x++)
    {
